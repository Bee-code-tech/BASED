...
library UserProfileLib {
    ...
    function addressZeroCheck(address _add) internal pure {
        if (_add == address(0)) revert BasedErrors.NOT_ALLOWED(address(0));
    }
    ...
    function _isUser(
        ...
        if (users[_caller].userAddress == address(0)) return false;
    ...
    function _followAUser(
        mapping(address => User) storage users,
        mapping(address => mapping(address => UserProfileLib.User))
            storage follows,
        address _me,
