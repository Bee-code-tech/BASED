...
library CommunityLib {
    ...
    function _createCommunity(
        ...
        bytes32 _commId = keccak256(
            abi.encode(_creator, _communityName, _description)
        );
    ...
    function _memberLeaves(
        ...
        if (!members[_commId][_member].isMember)
            revert BasedErrors.NOT_A_MEMBER(_member);
